odoo.define('hr_org_dep_chart.tests', function(require) {
    "use strict";
    ...
    QUnit.module('hr_org_dep_chart', {
        before: function() {
            this.data = {
                hr_department: {
                    fields: {
                        child_ids: { string: "one2many Subordinates field", type: "one2many", relation: 'hr_department' },
                    },
                    records: [{
                        id: 1,
                        child_ids: [],
        ...
        QUnit.test("hr org chart: empty render", async function(assert) {
            ...
            var form = await createView({
                ...
                mockRPC: function(route, args) {
                    if (route === 'hr_org_dep_chart/get_org_chart') {
                        assert.ok('department_id' in args, "it should have 'department_id' as argument");
                        return Promise.resolve({
                            children: [],
                            managers: [],
                            managers_more: false,
                        });
                    } else if (route === 'hr_org_dep_chart/get_redirect_model') {
                        return Promise.resolve('hr.department');
                    }
        ...
        QUnit.test("hr org chart: basic render", async function(assert) {
            ...
            var form = await createView({
                ...
                arch: '<form>' +
                    '<sheet>' +
                    '<div id="o_department_container"><div id="o_department_main">' +
        ...
        QUnit.test("hr org chart: basic manager render", async function(assert) {
            ...
            var form = await createView({
                ...
                mockRPC: function(route, args) {
                    if (route === 'hr_org_dep_chart/get_org_chart') {
                        assert.ok('department_id' in args, "should have 'department_id' as argument");
                        return Promise.resolve({
                            children: [{
                                direct_sub_count: 0,
                                indirect_sub_count: 0,
