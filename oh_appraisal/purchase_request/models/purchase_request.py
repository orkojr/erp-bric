...
class PurchaseRequest(models.Model):
    ...
    def _compute_is_editable(self):
        for rec in self:
            if rec.state in ("to_approve", "approved", "rejected", "approved_direction"):
    ...
    def _prepare_purchase_order(self, supplier_id, picking_type, group_id, company):
        if not supplier_id:
            raise UserError(_("Renseigner le fournisseur."))
...
class EmployeRequest(models.Model):
    ...
    def _prepare_purchase_request(self, request_by, service_id, service_demandeur, objet):
