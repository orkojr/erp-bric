...
class PurchaseRequestLineMakeAccountRecu(models.TransientModel):
    _name = "purchase.request.line.make.account.recu"
    _description = "Purchase Request Line Make Account recu"
    ...
    item_ids = fields.One2many(
        comodel_name="purchase.request.line.make.account.recu.item",
        inverse_name="wiz_recu_id",
        string="Items",
    )
    ...
    def _prepare_item(self, line):
        return {
            "line_id": line.id,
            "request_id": line.request_cout_id.request_id.id,
            "product_id": line.product_id.id,
            "name": line.product_id.name,
            "product_qty": line.quantite_piece,
    ...
    def _check_valid_request_line(self, request_line_ids):
        ...
        for line in self.env["purchase.request.line.cout"].browse(request_line_ids):
            if line.request_cout_id.request_id.state == "done":
                raise UserError(_("The purchase has already been completed."))
            if line.request_cout_id.request_id.state != "approved":
                raise UserError(
                    _("Purchase Request %s is not approved") % line.request_cout_id.request_id.name
                )

            # if line.purchase_state == "done":
            #     raise UserError(_("The purchase has already been completed."))

            line_company_id = line.request_cout_id.company_id and line.request_cout_id.company_id.id or False
            if company_id is not False and line_company_id != company_id:
                raise UserError(_("You have to select lines from the same company."))
            else:
                company_id = line_company_id

            line_picking_type = line.request_cout_id.request_id.picking_type_id or False
    ...
    def check_group(self, request_lines):
        if len(list(set(request_lines.mapped("request_cout_id.request_id.group_id")))) > 1:
            raise UserError(
                _(
    ...
    def get_items(self, request_line_ids):
        request_line_obj = self.env["purchase.request.line.cout"]
        items = []
    ...
    def default_get(self, fields):
        ...
        request_line_ids = []
        if active_model == "purchase.request.line.cout":
            request_line_ids += self.env.context.get("active_ids", [])
        elif active_model == "purchase.request.line":
            request_ids = self.env.context.get("active_ids", False)
            request_line_ids += (
                self.env[active_model].browse(request_ids).mapped("line_cout_ids.id")
            )
        if not request_line_ids:
            return res
        res["item_ids"] = self.get_items(request_line_ids)
        request_lines = self.env["purchase.request.line.cout"].browse(request_line_ids)
    ...
    def _prepare_account_move_line(self, item, move=False):
        ...
        #qty = max(qty, min_qty)
        date_required = item.line_id.request_cout_id.date_required
        vals = {
            "name": product.name,
            ...
            "analytic_account_id": item.line_id.request_cout_id.analytic_account_id.id,
            "purchase_request_lines": [(4, item.line_id.request_cout_id.id)],
            
            
        }
        #"move_dest_ids": [(4, x.id) for x in item.line_id.move_dest_ids],
        if item.line_id.request_cout_id.analytic_tag_ids:
    ...
    def _get_order_line_search_domain(self, order, item):
        ...
        order_line_data = [
            ...
            ("analytic_account_id", "=", item.line_id.request_cout_id.analytic_account_id.id or False),
        ]
...
class PurchaseRequestLineMakeAccountRecuItem(models.TransientModel):
    _name = "purchase.request.line.make.account.recu.item"
    _description = "Purchase Request Line Make Account Recu Item"

    wiz_recu_id = fields.Many2one(
        comodel_name="purchase.request.line.make.account.recu",
        string="Wizard",
        required=True,
        ondelete="cascade",
        readonly=True,
    ...
    line_id = fields.Many2one(
        comodel_name="purchase.request.line.cout", string="Purchase Request Line"
    )
    request_id = fields.Many2one(
        comodel_name="purchase.request.line",
        related="line_id.request_cout_id",
        ...
        if self.product_id:
            ...
            sup_info_id = self.env["product.supplierinfo"].search(
                [
                    ...
                    ("name", "=", self.wiz_recu_id.supplier_id.id),
                ]
            )
            if sup_info_id:
