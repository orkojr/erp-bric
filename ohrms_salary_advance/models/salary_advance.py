...
class SalaryAdvancePayment(models.Model):
    ...
    _description = "Avances sur salaire"
    ...
    employee_id = fields.Many2one('hr.employee', string='Employe', default=_default_employee_id, required=True, help="Employee")
    date = fields.Date(string='Date', required=True, default=lambda self: fields.Date.today(), help="Submit date")
    reason = fields.Text(string='Raison', required=True, help="Reason")
    ...
    payment_method = fields.Many2one('account.journal', string='Methode de paiement')
    ...
    department = fields.Many2one('hr.department', string='Service')
    state = fields.Selection([('draft', 'Brouillon'),
                              ('submit', 'Soumis'),
                              ('waiting_approval', 'Attente Approbation'),
                              ('approve', 'Approuve'),
                              ('cancel', 'Annule'),
                              ('reject', 'Rejete')], string='Statut', default='draft', track_visibility='onchange')
    debit = fields.Many2one('account.account', string='Compte debit')
    credit = fields.Many2one('account.account', string='Compte credit')
    ...
    def approve_request(self):
        ...
        for each_advance in salary_advance_search:
            ...
            if current_month == existing_month:
                raise UserError('Une avance peut etre demandee une seule fois dans un mois')
        if not self.employee_contract_id:
            raise UserError('Definir le contrat de lemploye')
        struct_id = self.employee_contract_id.struct_id
        adv = self.advance
        amt = self.employee_contract_id.wage
        if adv > amt and not self.exceed_condition:
            raise UserError('Le montant de votre avance est superieur a celui alloue')
        ...
        payslip_obj = self.env['hr.payslip'].search([('employee_id', '=', self.employee_id.id),
                                                     ...
                                                     ('date_to', '>=', self.date)])
        if payslip_obj:
            raise UserError("Le salaire de ce mois est deja calcule")
            ...
            if current_month == slip_moth + 1:
                ...
                if current_day - slip_day < struct_id.advance_date:
                    raise exceptions.Warning(
                        _('La demande peut etre faite "%s" Jours apres le salaire du mois precedent') % struct_id.advance_date)
